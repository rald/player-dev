...
typedef struct {
    ...
    int indices[MAX_PLAYLIST_SIZE];  // Track order (original vs shuffled)
    int count;
    bool shuffled;
} Playlist;
...
bool load_playlist(const char* playlist_file, Playlist* playlist) {
    ...
    while (fgets(playlist->filenames[playlist->count], MAX_PATH_LEN, file)) {
        ...
        if (strlen(playlist->filenames[playlist->count]) > 0) {
            playlist->indices[playlist->count] = playlist->count;  // Default order
            playlist->count++;
...
int main(int argc, char **argv) {
    ...
    if (argc < 2 || (argc == 2 && strcmp(argv[1], "-s") == 0)) {
        ...
        printf("Controls: N=Next, P=Prev, Space=Next, Enter=Quit\n");
        return -1;
    }
    ...
    printf("N=Next, P=Prev, Space=Next, Enter=Quit\n\n");
