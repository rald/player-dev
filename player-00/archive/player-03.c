...
int main(int argc, char **argv) {
    ...
    while (!quit) {
        ...
        printf("[%d/%d] %s\n", current_track+1, playlist.count, playlist.filenames[track_idx]);
        
        result = ma_sound_init_from_file(&engine, playlist.filenames[track_idx], 0, NULL, NULL, &sound);
        if (result != MA_SUCCESS) {
        ...
        // Play until end OR keypress
        bool go_next = false;
        bool go_prev = false;
        while (ma_sound_is_playing(&sound) && !quit && !go_next && !go_prev) {
